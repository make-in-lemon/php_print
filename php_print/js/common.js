    ...
    $.getJSON(SITEURL+'/index.php?act=cart&op=del&cart_id='+cart_id+'&goods_id='+goods_id+'&callback=?', function(result){
        if(result.state){
            ...
            if(result.quantity == 0){
    	      	...
    	      	obj.find('.incart-goods').html(html);
    	      	obj.find('.total-price').html('');
		        obj.find('.addcart-goods-num').html('0');
...
(function($) {
    $.show_nc_login = function(options) {
        ...
        login_dialog_html.append('<form class="bg" method="post" id="ajax_login" action="'+APP_SITE_URL+settings.action+'"></form>').find('form')
        	...
        	.append('<dl><dt>邮箱/手机</dt><dd><input type="text" name="user_name" autocomplete="off" class="text"></dd></dl>')
        	...
        	.append('<ul><li>›&nbsp;如果您没有登录帐号，请先<a class="register" href="'+SHOP_SITE_URL+'/index.php?act=login&amp;op=register">注册会员</a>然后登录</li><li>›&nbsp;如果您忘记密码，可申请<a class="forget" href="'+SHOP_SITE_URL+'/index.php?act=login&amp;op=forget_password">找回密码</a></li></ul>')
        ...
        html_form("form_dialog_login", "登&nbsp;录", login_dialog_html, 360);
    };
 ...
 *
 * JAVASCRIPT part
 * <script type="text/javascript" src="<?php echo PUBLIC_SITE_URL;?>/js/qtip/jquery.qtip.min.js"></script>
 * <link href="<?php echo PUBLIC_SITE_URL;?>/js/qtip/jquery.qtip.min.css" rel="stylesheet" type="text/css">
		                ...
		                success: function(data) {
		                	if(data){
		                		...
		                		$('<dt class="member-id"></dt>').append('<i class="sex'+data.sex+'"></i>')
		                			.append('<a href="'+SHOP_SITE_URL+'/index.php?act=member_snshome&mid='+data.id+'" target="_blank">'+data.nickname+'</a>'+(data.truename != ''?'('+data.truename+')':''))
		                			.appendTo(_dl);
		                		// avatar
    ...
    $.fn.nc_region = function(options) {
        ...
        return this.each(function() {
            ...
            if($inputArea.val() === '') {
                ...
                var $region_btn = $('<input type="button" class="edit-botton" value="修改" />');
